...
const InlineMath = dynamic(() => import('./Formula').then(mod => mod.InlineMath), { ssr: false });
const DisplayMath = dynamic(() => import('./Formula').then(mod => mod.DisplayMath), { ssr: false });
const MathJax = dynamic(() => import('./MathJax'), { ssr: false });

export default function TestMathContent() {
  return (
    <Layout title="数学公式测试">
      <MathJax />
